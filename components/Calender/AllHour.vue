        ...
        <div v-for="j in 8" :key="j" class="child">
          ...
          >
            <div :data-index="j" :data-hour="i - 1">
              <p
                v-for="event in array[i - 1][j - 2]"
                :key="event.id"
                :style="{ backgroundColor: event.backgroundColor }"
                class="event_para"
                @click="toggleTaskModal(event)"
              >
                {{ `${event.eventUser} : ${event.what}` }}
              </p>
            </div>
          </div>
        </div>
...
onBeforeUpdate(() => {
  array.value = Array(24)
    .fill(null)
    .map(() => Array(7).fill([]));
  linkEvent();
  const currentTour = new Date().getHours();
  const row = document.querySelector(
    `.container:nth-child(${currentTour + 1})`
  );
